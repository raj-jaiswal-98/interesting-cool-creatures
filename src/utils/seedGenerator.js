 ...
 * @param {number} creatureListLength - Total number of available creatures
 * @param {string} [customDateStr] - Optional YYYY-MM-DD date override (defaults to current UTC)
 * @returns {number} Deterministic index in range [0, creatureListLength - 1]
 */
export function getDailyCreatureIndex(creatureListLength, customDateStr) {
 ...
 * @param {Array} creatureList
 * @param {string} [customDateStr]
 * @returns {Object|null}
 */
export function getDailySpotlightCreature(creatureList, customDateStr) {
 ...
 */
export function getTimeUntilNextSpotlight() {
  const now = new Date();
